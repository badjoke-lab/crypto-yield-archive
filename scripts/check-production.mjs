...
const sourceCommit=process.env.CYA_EXPECTED_SOURCE_COMMIT?.trim()||null;
const attempts=Number(process.env.CYA_SMOKE_ATTEMPTS||12);
const delay=Number(process.env.CYA_SMOKE_DELAY_MS||15000);
let lastError;
for(let i=1;i<=attempts;i+=1){
  try{const result=await checkProduction(base,commit,sourceCommit);console.log(JSON.stringify({...result,attempt:i},null,2));process.exit(0)}
